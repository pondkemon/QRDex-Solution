...
open class QrGenaraterService {
    public init() {}
    public func textToImage( text: String ) -> UIImage? {
        let myString = text
        // 2
    ...
    public func getQRSting(tel: String, amount: Double) -> String {
        var base = "00020101021129370016A00000067701011101130066"+tel+"5802TH54"
        let amountStr = String(format: "%01.2f", amount)
    ...
    private let CRC16_XMODEM_TABLE: [UInt16] = [
    ...
    private func _crc16(data: Data, crc: UInt16, table: [UInt16]) -> UInt16 {
      var crc = crc
      for byte in data {
        let index = Int((crc >> 8) ^ UInt16(byte))
        crc = ((crc << 8) & 0xff00) ^ table[index]
      }
      return crc & 0xffff
    }
